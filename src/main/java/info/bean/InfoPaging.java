...
public class InfoPaging {
	...
	public void makePagingHTML() {
		...
		if(startPage > pageBlock) {
			pagingHTML.append("<a id='paging' href='infomation.do?pg="+(startPage-1)+"'>이전</a>");
		}
		
		for(int i=startPage; i<=endPage; i++) {
			if(i == currentPage) {//현재페이지라면
				pagingHTML.append("<a id='currentPaging' style='padding-right:5px;' href='infomation.do?pg="+i+"'>"+i+"</a>");
			}else {
				pagingHTML.append("<a id='paging' style='padding-right:5px;' href='infomation.do?pg="+i+"'>"+i+"</a>");
			}
		}
		if(endPage < totalP) {
			pagingHTML.append("<a id='paging' href='infomation.do?pg="+(endPage+1)+"'>다음</a>");
		}
	}
